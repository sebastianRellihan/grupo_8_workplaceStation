const { category } = require("../../src/database/models");

// Descripción y scope del caso de prueba
describe("CRUD básico del modelo category", () => {
    // Objeto base sobre el cual testear las operaciones CRUD a BD
    const testCategory = {
        name : "Cajoneras"
    }
    // Objeto "options" para consultas SELECT
    let options = {
        where : {
            name : testCategory.name
        }
    }

    // it = prueba unitaria sobre una o un conjunto de funciones concretas
    it("Create", done => {
        category.create(testCategory) // Retorna una promesa
            .then(created => {
                // Determina el caso de éxito de la prueba comparando dos valores
                expect(created.name).toBe(testCategory.name);
                done(); // Avisa a jasmine que el pedido asincrónico (promesa) fue finalizado
            }).catch(error => {
                console.log(error);
                done();
            });
    });

    it("Read", done => {
        category.findOne(options)
            .then(result => {
                // Checkea la integridad de los datos creados
                expect(result.name).toBe(testCategory.name);
                done();
            })
            .catch(error => {
                console.log(error);
                done();
            });
    });

    it("Update", done => {
        const newName = "Plegables";
        category.update(
            { name : newName },
            options
        )
        .then(() => {
            // Debido a que el registro tiene un nuevo nombre, el valor de
            // la clausula WHERE tiene que cambiar para poder buscar el registro en BD
            options.where.name = newName;
            // Busca por el nuevo nombre y retorna una promesa
            return category.findOne(options); 
        })
        .then(result => {
            // Chequea que el cambio sea efectivo
            expect(result.name).toBe(newName);
            done();
        })
        .catch(error => {
            console.log(error);
            done();
        })
    });

    it("Destroy", done => {

        category.destroy(options)
        .then(() => {
            // Busca el registro en BD
            return category.findOne(options);
        })
        .then(result => {
            // Checkea que efectivamente se haya borrado
            expect(result).toBe(null);
            done();
        })
        .catch(error => {
            console.log(error);
            done();
        })
    });

});